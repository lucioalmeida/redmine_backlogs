...
RB.Model = RB.Object.create({
  ...
  edit: function(){
    ...
    this.$.find('.editable').each(function(index){
      ...
      var fieldRequired = field.attr('fieldrequired') ? field.attr('fieldrequired'): "false";
      var input;

      var divLabel = RB.$(document.createElement("div"));
      divLabel.appendTo(editor);
      RB.$(document.createElement("span")).text(fieldLabel).appendTo(divLabel);
      if (fieldRequired == "true") {
          RB.$(document.createElement("span")).attr("class","required").text("*").appendTo(divLabel);
      }
      var element = RB.$('#' + fieldName + '_options');
      input = fieldType=='select' && element.length == 1  ? element.clone(true) : RB.$(document.createElement(fieldType));
      if (element.length == 0 && fieldType == 'select' && field.children('option').length > 0) {
          input.append(field.children('option'));
      }
      input.removeAttr('id');
