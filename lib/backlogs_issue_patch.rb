    ...
    module InstanceMethods
      ...
      def backlogs_before_save
        if Backlogs.configured?(project)
          if (self.is_task? || self.story)
            ...
            self.start_date = Date.today if self.start_date.blank? && self.status_id != self.default_status.id
      ...
      def backlogs_after_save
        ...
        if self.is_story?
          ...
          if tasklist.size > 0
            task_ids = '(' + tasklist.collect{|task| RbStory.connection.quote(task.id)}.join(',') + ')'
            RbStory.connection.execute("update issues set
                                updated_on = #{RbStory.connection.quote(self.updated_on)}, fixed_version_id = #{RbStory.connection.quote(self.fixed_version_id)}, tracker_id = #{RbTask.tracker}
